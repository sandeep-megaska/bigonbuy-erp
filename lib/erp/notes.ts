...
export const noteListRowSchema = z.object({
  ...
  note_no: z.string().nullable().optional(),
  party_type: z.enum(["vendor", "customer"]),
  note_kind: z.enum(["credit", "debit", "CN", "DN"]),
  status: z.enum(["draft", "approved", "cancelled"]).or(z.string()),
  note_date: z.string(),
  party_id: z.string().uuid().nullable(),
  party_name: z.string(),
  currency: z.string(),
  ...
  tax_total: z.coerce.number(),
  total: z.coerce.number(),
  reference_invoice_number: z.string().nullable().optional(),
  reference_invoice_date: z.string().nullable().optional(),
