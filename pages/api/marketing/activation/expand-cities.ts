...
type ApiResponse = ActivationExpandCityRow[] | { error: string };
...
export default async function handler(req: NextApiRequest, res: NextApiResponse<ApiResponse>) {
  ...
  if (error) {
    return res.status(400).json({ error: error.message || "Failed to load activation expand cities" });
  }

  return res.status(200).json((data ?? []) as ActivationExpandCityRow[]);
}
