...
type ApiResponse = { rows: ReduceSkuRow[] } | { error: string };
...
export default async function handler(req: NextApiRequest, res: NextApiResponse<ApiResponse>) {
  ...
  if (error) {
    return res.status(400).json({ error: error.message || "Failed to load reduce SKUs" });
  }

  return res.status(200).json({ rows: (data ?? []) as ReduceSkuRow[] });
}
