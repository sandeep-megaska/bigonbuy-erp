...
export default async function handler(req: NextApiRequest, res: NextApiResponse<string | { error: string }>) {
  ...
  const { data, error } = await auth.supabase.rpc("erp_mkt_audience_export_vip_buyers_180d_v1", {
    p_limit: parseLimit(req.query.limit),
  });

  if (error) {
    return res.status(400).json({ error: error.message || "Failed to export VIP buyers audience" });
  }

  const csv = toCsv((data ?? []) as AudienceCsvRow[]);
  res.setHeader("Content-Type", "text/csv; charset=utf-8");
  res.setHeader("Content-Disposition", 'attachment; filename="meta_audience_vip_buyers_180d.csv"');
