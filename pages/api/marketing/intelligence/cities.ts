...
const ALLOWED_SORTS = new Set(["conversion_index", "orders_count", "revenue", "aov", "updated_at"]);
...
export default async function handler(req: NextApiRequest, res: NextApiResponse<ApiResponse>) {
  ...
  const rawSort = typeof req.query.sort === "string" ? req.query.sort : "conversion_index";
  const sort = ALLOWED_SORTS.has(rawSort) ? rawSort : "conversion_index";
  ...
  if (error) {
    return res.status(400).json({ ok: false, error: error.message || "Failed to load city intelligence" });
  }
