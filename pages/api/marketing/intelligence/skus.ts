...
export default async function handler(req: NextApiRequest, res: NextApiResponse<ApiResponse>) {
  ...
  const rawSort = typeof req.query.sort === "string" ? req.query.sort : "profitability_score";
  const sort = ALLOWED_SORTS.has(rawSort) ? rawSort : "profitability_score";
  ...
  if (error) {
    return res.status(400).json({ ok: false, error: error.message || "Failed to load SKU intelligence" });
  }
