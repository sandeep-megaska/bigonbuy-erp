...
export default async function handler(
  ...
  try {
    ...
    const orderIdKeys = ["orderid", "amazonorderid"];
    const purchaseDateKeys = ["purchasedate", "orderdate", "orderdateutc", "purchasedatetime"];
    const statusKeys = ["orderstatus", "status"];
    const skuKeys = ["sku", "sellersku", "merchantsku"];
    const asinKeys = ["asin", "asin1"];
    const fnskuKeys = ["fnsku", "fnskuid"];
    const titleKeys = ["title", "productname", "itemname"];
    const qtyKeys = ["quantity", "quantityordered", "quantitypurchased"];
    const itemAmountKeys = ["itemprice", "itempriceamount"];
    const itemTaxKeys = ["itemtax", "itemtaxamount"];
    const shippingAmountKeys = ["shippingprice", "shippingamount", "shippingpriceamount"];
    const shippingTaxKeys = ["shippingtax", "shippingtaxamount"];
    ...
    const currencyKeys = ["currency", "currencycode"];
    const shipStateKeys = ["shipstate", "shippingaddressstate", "shippingstate"];
    const shipCityKeys = ["shipcity", "shippingaddresscity", "shippingcity"];
    const shipPostalKeys = ["shippostalcode", "shippingaddresspostalcode", "postalcode"];
    const buyerEmailKeys = ["buyeremail", "buyeremailaddress"];
    const orderItemIdKeys = ["orderitemid", "orderitemidentifier"];
    const orderTotalKeys = ["ordertotal", "orderamount", "ordertotalamount"];
    ...
    rows.slice(1).forEach((row, index) => {
      ...
      const orderTotal = parseNumber(getValue(row, orderTotalKeys));
      ...
      draftItems.push({
        ...
        order_item_id: orderItemId ?? orderItemKey,
      ...
      const agg = orderAggregates.get(orderId) ?? {
        ...
        ship_postal_code: shipPostal,
        currency,
        explicit_total: orderTotal > 0 ? orderTotal : null,
        calculated_total: 0,
      };

      if (orderStatus && !agg.order_status) agg.order_status = orderStatus;
      if (buyerEmail && !agg.buyer_email) agg.buyer_email = buyerEmail;
      if (shipState && !agg.ship_state) agg.ship_state = shipState;
      if (shipCity && !agg.ship_city) agg.ship_city = shipCity;
      if (shipPostal && !agg.ship_postal_code) agg.ship_postal_code = shipPostal;
      if (currency && !agg.currency) agg.currency = currency;
      if (purchaseDate && (!agg.purchase_date || purchaseDate < agg.purchase_date)) {
        agg.purchase_date = purchaseDate;
      }
      if (orderTotal > 0) {
        agg.explicit_total = orderTotal;
      }

      const lineTotal = itemAmount + itemTax + shippingAmount + shippingTax - promoDiscount;
      agg.calculated_total += lineTotal;

      orderAggregates.set(orderId, agg);
    });
    ...
    const itemsInsertPayload = draftItems.map((item) => {
    ...
    for (const payloadChunk of chunk(itemsInsertPayload, 500)) {
      const { error: itemsError } = await auth.serviceClient
        .from("erp_amazon_order_items")
        .upsert(payloadChunk, {
          onConflict: "company_id,marketplace_id,amazon_order_id,order_item_key",
        });
    ...
    const ordersInsertPayload = Array.from(orderAggregates.values()).map((order) => ({
      company_id: auth.companyId,
      marketplace_id: marketplaceId,
      ...
      order_total: order.explicit_total ?? order.calculated_total,
    ...
    for (const payloadChunk of chunk(ordersInsertPayload, 500)) {
      const { error: ordersError } = await auth.serviceClient
        .from("erp_amazon_orders")
        .upsert(payloadChunk, {
          onConflict: "company_id,marketplace_id,amazon_order_id",
        });
 ...
 const { data: factsResult, error: factsError } = await auth.serviceClient.rpc(
  "erp_amazon_order_facts_upsert_from_oms_v4",
  {
