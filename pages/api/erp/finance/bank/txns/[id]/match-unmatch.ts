...
export default async function handler(req: NextApiRequest, res: NextApiResponse<ApiResponse>) {
  ...
  const payload = (req.body ?? {}) as { reason?: string | null };
  ...
  try {
    ...
    const { data, error } = await userClient.rpc("erp_bank_txn_match_unmatch", {
    ...
    if (error) {
      return res.status(400).json({
        ok: false,
        error: error.message || "Failed to unmatch bank transaction",
