...
export default async function handler(req: NextApiRequest, res: NextApiResponse<ApiResponse>) {
  ...
  const payload = (req.body ?? {}) as { dateWindowDays?: number };
  ...
  try {
    ...
    const { data, error } = await userClient.rpc("erp_bank_txn_match_suggest_razorpay", {
      p_bank_txn_id: bankTxnId,
      p_date_window_days: payload.dateWindowDays ?? 3,
    });

    if (error) {
      return res.status(400).json({
        ok: false,
        error: error.message || "Failed to load Razorpay match suggestions",
