...
export default async function handler(req: NextApiRequest, res: NextApiResponse<ApiResponse>) {
  ...
  const day = parseDateParam(req.query.day);
  if (!day) {
  ...
  try {
    ...
    const { data, error } = await userClient.rpc("erp_shopify_sales_day_posting_preview", {
      p_day: day,
    });
    ...
    if (error) {
      return res.status(400).json({
        ok: false,
        error: error.message || "Failed to load Shopify daily posting preview",
