...
export default async function handler(req: NextApiRequest, res: NextApiResponse<ApiResponse>) {
  ...
  const cashflowGroup = parseOptionalString(req.query.cashflow_group);
  const cashflowSubgroup = parseOptionalString(req.query.cashflow_subgroup);
  ...
  if (!cashflowGroup || !cashflowSubgroup) {
    return res.status(400).json({ ok: false, error: "cashflow_group and cashflow_subgroup are required." });
  }
  ...
  try {
    ...
    const { data, error } = await userClient.rpc("erp_fin_cash_flow_drilldown", {
      p_company_id: companyId,
      p_from: from,
      p_to: to,
      p_cashflow_group: cashflowGroup,
      p_cashflow_subgroup: cashflowSubgroup,
    });

    if (error) {
      return res.status(400).json({
        ok: false,
        error: error.message || "Failed to load cash flow drilldown",
