...
export default async function handler(req: NextApiRequest, res: NextApiResponse<ApiResponse>) {
  ...
  const accountId = parseOptionalString(req.query.account_id);
  ...
  if (!accountId) {
    return res.status(400).json({ ok: false, error: "Missing account_id." });
  }
  if (!from) {
  ...
  try {
    ...
    const { data, error } = await userClient.rpc("erp_fin_account_ledger", {
    ...
    if (error) {
      return res.status(400).json({
        ok: false,
        error: error.message || "Failed to load account ledger",
