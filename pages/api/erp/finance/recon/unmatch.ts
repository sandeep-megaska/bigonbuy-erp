...
export default async function handler(req: NextApiRequest, res: NextApiResponse<ApiResponse>) {
  ...
  const payload = (req.body ?? {}) as { bank_txn_id?: string };
  if (!payload.bank_txn_id) {
    return res.status(400).json({ ok: false, error: "bank_txn_id is required" });
  }
  ...
  try {
    ...
    const { data, error } = await userClient.rpc("erp_bank_recon_unmatch", {
