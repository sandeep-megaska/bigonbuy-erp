...
export default async function handler(req: NextApiRequest, res: NextApiResponse<ApiResponse>) {
  ...
  const fileName = typeof file_name === "string" ? sanitizeFileName(file_name) : "photo.jpg";
  const contentType = typeof content_type === "string" ? content_type : "image/jpeg";
  ...
  const path = `photos/${employeeId}/${Date.now()}-${fileName}`;
