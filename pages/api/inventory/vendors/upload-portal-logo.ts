...
export default async function handler(req: NextApiRequest, res: NextApiResponse<Resp>) {
  ...
  const { vendor_id, file_base64, mime_type, filename, remove } = (req.body ?? {}) as Record<string, unknown>;
  ...
  const { data: vendor, error: vendorError } = await adminClient
    .from("erp_vendors")
    .select("id, company_id, portal_logo_path")
    .eq("id", vendorId)
    .eq("company_id", companyId)
    .maybeSingle();
  ...
  if (Boolean(remove)) {
    if (vendor.portal_logo_path) {
      await adminClient.storage.from(BUCKET).remove([vendor.portal_logo_path]);
    }
    const { error: updateError } = await adminClient
      .from("erp_vendors")
      .update({ portal_logo_path: null, portal_logo_updated_at: new Date().toISOString() })
      .eq("id", vendorId)
      .eq("company_id", companyId);

    if (updateError) {
      return res.status(400).json({ ok: false, error: updateError.message || "Failed to clear vendor logo" });
    }
  ...
  const fileName = typeof filename === "string" ? filename.trim() : "logo.png";
  ...
  const path = `vendors/${companyId}/${vendorId}/logo.${extension}`;
  ...
  if (uploadError) {
    return res.status(400).json({ ok: false, error: uploadError.message || "Failed to upload vendor logo" });
  }

  const { error: updateError } = await adminClient
    .from("erp_vendors")
    .update({ portal_logo_path: path, portal_logo_updated_at: new Date().toISOString() })
    .eq("id", vendorId)
    .eq("company_id", companyId);

  if (updateError) {
    return res.status(400).json({ ok: false, error: updateError.message || "Failed to save vendor logo path" });
  }
