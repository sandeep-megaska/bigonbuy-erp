...
export default async function handler(req: NextApiRequest, res: NextApiResponse<ApiResponse>) {
  ...
  try {
    ...
    if (!isManager) {
      return res.status(403).json({ ok: false, error: "Not authorized: owner/admin/hr only" });
    }

    const { data: employees, error: employeesError } = await userClient.rpc("erp_list_employees");
    if (employeesError) {
      return res.status(500).json({
        ok: false,
        error: employeesError.message || "erp_list_employees failed",
        details: employeesError.details || employeesError.hint || employeesError.code,
      });
    }

    const employeeCount = Array.isArray(employees) ? employees.length : 0;
    return res.status(200).json({ ok: true, manager: !!isManager, employee_count: employeeCount });
  } catch (error) {
