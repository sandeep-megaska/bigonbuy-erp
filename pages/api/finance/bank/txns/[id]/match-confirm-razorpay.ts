...
export default async function handler(req: NextApiRequest, res: NextApiResponse<ApiResponse>) {
  ...
  const payload = (req.body ?? {}) as {
    settlementDbId?: string | null;
    confidence?: string | null;
    notes?: string | null;
  };

  const settlementDbId = payload.settlementDbId ? String(payload.settlementDbId).trim() : "";
  if (!settlementDbId) {
    return res.status(400).json({ ok: false, error: "settlementDbId is required" });
  }
  ...
  try {
    ...
    const { data, error } = await userClient.rpc("erp_bank_txn_match_confirm", {
      p_bank_txn_id: bankTxnId,
      p_settlement_db_id: settlementDbId,
    ...
    if (error) {
      return res.status(400).json({
        ok: false,
        error: error.message || "Failed to confirm Razorpay match",
