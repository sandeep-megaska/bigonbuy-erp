...
export default async function handler(req: NextApiRequest, res: NextApiResponse<ApiResponse>) {
  ...
  const date = parseDate(parseOptionalString(req.query.date));
  ...
  try {
    ...
    const { data, error } = await userClient.rpc("erp_fin_period_status", {
    ...
    if (error) {
      return res.status(400).json({
        ok: false,
        error: error.message || "Failed to load period status",
