...
export default async function handler(req: NextApiRequest, res: NextApiResponse<ApiResponse>) {
  ...
  try {
    ...
    const rpcPayload: Record<string, unknown> = { p_vendor_payment_id: payload.paymentId };
    if (canBypassMakerChecker(roleKey)) {
      rpcPayload.p_use_maker_checker = false;
    }

    const { data, error } = await userClient.rpc("erp_ap_vendor_payment_approve", rpcPayload);

    if (error) {
      return res.status(400).json({
        ok: false,
        error: error.message || "Failed to post vendor payment",
