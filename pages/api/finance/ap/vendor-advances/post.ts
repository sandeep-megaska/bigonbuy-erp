...
export default async function handler(req: NextApiRequest, res: NextApiResponse<ApiResponse>) {
  ...
  if (!payload.advanceId) {
    return res.status(400).json({ ok: false, error: "advanceId is required" });
  }
  ...
  try {
    ...
    const rpcPayload: Record<string, unknown> = { p_advance_id: payload.advanceId };
    if (canBypassMakerChecker(roleKey)) {
      rpcPayload.p_use_maker_checker = false;
    }

    const { data, error } = await userClient.rpc("erp_ap_vendor_advance_approve_and_post", rpcPayload);

    if (error) {
      return res.status(400).json({
        ok: false,
        error: error.message || "Failed to post vendor advance",
