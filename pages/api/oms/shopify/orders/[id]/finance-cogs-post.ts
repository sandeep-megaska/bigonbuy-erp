...
export default async function handler(req: NextApiRequest, res: NextApiResponse<ApiResponse>) {
  ...
  try {
    ...
    const payload = (req.body ?? {}) as { idempotencyKey?: string | null };

    const { data: journalId, error: postError } = await userClient.rpc("erp_sales_finance_cogs_post", {
      p_order_id: orderId,
    ...
    if (postError) {
      return res.status(400).json({
        ok: false,
        error: postError.message || "Failed to post COGS journal",
