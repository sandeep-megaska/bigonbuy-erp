...
export default async function handler(req: NextApiRequest, res: NextApiResponse<ApiResponse>) {
  ...
  if (error) {
    return res.status(500).json({ ok: false, error: error.message || "Failed", details: error.details || error.hint || error.code });
  }

  const payload = (data ?? {}) as any;
  if (!payload.ok) {
    return res.status(400).json({ ok: false, error: payload.error || "Failed" });
  }
