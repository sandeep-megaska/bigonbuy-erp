...
export default async function handler(req: NextApiRequest, res: NextApiResponse<Resp>) {
  ...
  const { data, error } = await admin.rpc("erp_mfg_prod_checkpoints_list_v1", {
    p_company_id: auth.session.company_id,
    p_vendor_id: auth.session.vendor_id,
  });

  if (error) return res.status(400).json({ ok: false, error: error.message || "Failed to load checkpoints" });
