...
export default async function handler(req: NextApiRequest, res: NextApiResponse<Resp>) {
  ...
  if (!cartonId) return res.status(400).json({ ok: false, error: "carton_id is required" });

  const anon = createAnonClient(supabaseUrl, anonKey);
  const { data, error } = await anon.rpc("erp_mfg_asn_scan_undo_last_v1", {
    p_session_token: token,
    p_carton_id: cartonId,
  });

  if (error) return res.status(400).json({ ok: false, error: error.message || "Failed to undo scan" });
