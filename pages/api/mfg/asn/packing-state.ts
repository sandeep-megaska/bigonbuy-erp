...
export default async function handler(req: NextApiRequest, res: NextApiResponse<Resp>) {
  ...
  const { data, error } = await anon.rpc("erp_mfg_asn_packing_state_v1", {
    p_session_token: token,
    p_asn_id: asnId,
  });

  if (error) return res.status(400).json({ ok: false, error: error.message || "Failed to load packing state" });
