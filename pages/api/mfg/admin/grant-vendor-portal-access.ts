...
export default async function handler(req: NextApiRequest, res: NextApiResponse<ApiResponse>) {
  ...
  const { vendor_id, email, role_key } = (req.body ?? {}) as Record<string, unknown>;
  const vendorId = typeof vendor_id === "string" ? vendor_id : "";
  const normalizedEmail = typeof email === "string" ? email.trim().toLowerCase() : "";
  const normalizedRole = typeof role_key === "string" && role_key.trim() ? role_key.trim() : "vendor";

  if (!vendorId || !normalizedEmail) {
    return res.status(400).json({ ok: false, error: "vendor_id and email are required" });
  }
  ...
  if (error) {
    return res.status(400).json({
      ok: false,
      error: error.message || "Failed to grant vendor portal access",
