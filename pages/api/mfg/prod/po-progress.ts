...
export default async function handler(req: NextApiRequest, res: NextApiResponse<Resp>) {
  ...
  const poId = String(Array.isArray(req.query.po_id) ? req.query.po_id[0] : req.query.po_id || "").trim();
  if (!poId) return res.status(400).json({ ok: false, error: "po_id is required" });
  ...
  const { data, error } = await admin.rpc("erp_mfg_po_line_progress_get_v1", {
    p_company_id: auth.session.company_id,
    p_vendor_id: auth.session.vendor_id,
    p_po_id: poId,
  });

  if (error) return res.status(400).json({ ok: false, error: error.message || "Failed to load PO progress" });
