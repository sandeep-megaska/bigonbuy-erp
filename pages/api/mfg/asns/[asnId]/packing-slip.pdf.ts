...
import { generatePackingSlipPdf, type AsnPrintData } from "../../../../../lib/mfg/asnPrintPdf";
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  const format = (Array.isArray(req.query.format) ? req.query.format[0] : req.query.format) === "a4" ? "a4" : "slip";
  ...
  const pdf = await generatePackingSlipPdf(payload, format);
  const fileAsn = payload?.asn?.asn_no || payload?.asn?.id || asnId;

  res.setHeader("Content-Type", "application/pdf");
  res.setHeader("Content-Disposition", `inline; filename="ASN_${fileAsn}_packing_slip.pdf"`);
