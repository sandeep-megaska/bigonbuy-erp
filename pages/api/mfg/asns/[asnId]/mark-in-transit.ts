...
export default async function handler(req: NextApiRequest, res: NextApiResponse<Resp>) {
  ...
  const { data, error } = await anon.rpc("erp_mfg_asn_mark_in_transit_v1", {
    p_session_token: token,
    p_asn_id: asnId,
    p_remarks: String(req.body?.remarks || "").trim() || null,
  });

  if (error) return res.status(400).json({ ok: false, error: error.message || "Failed to mark in transit" });
