...
export default async function handler(req: NextApiRequest, res: NextApiResponse<Resp>) {
  ...
  const bucket = String(req.query.bucket || "WEEK").toUpperCase();

  const anon = createAnonClient(supabaseUrl, anonKey);
  const { data, error } = await anon.rpc("erp_mfg_vendor_perf_trends_v1", {
  ...
  if (error) {
    return res.status(400).json({ ok: false, error: error.message || "Failed to load trends" });
  }
