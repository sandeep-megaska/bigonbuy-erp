...
export default function MyPayslipDetailPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (!context.companyId || !context.employeeId) {
        setErr(context.membershipError || "No employee profile is linked to this account.");
        setLoading(false);
        return;
      }
      setLoading(false);
  ...
  if (!ctx?.companyId || !ctx?.employeeId) {
    return (
      <div style={{ padding: 24, fontFamily: "system-ui" }}>
        <h1>Payslip</h1>
        <p style={{ color: "#b91c1c" }}>{err || "No employee profile is linked to this account."}</p>
    ...
    return (
      <div style={{ padding: 24, fontFamily: "system-ui" }}>
        ...
        <div style={{ marginTop: 12, display: "flex", gap: 10, flexWrap: "wrap" }}>
          <a href="/erp/my/payslips" style={buttonStyle}>Back to Payslips</a>
          <button onClick={handleSignOut} style={buttonStyle}>Sign Out</button>
        </div>
      </div>
    );
  ...
  return (
    <PayslipDetail
      ...
      backHref="/erp/my/payslips"
      backLabel="Back to Payslips"
      onDownload={handleDownload}
      contextLabel={`Signed in as ${ctx?.email}`}
    />
  );
}
