...
export default function HrEmployeeGendersPage() {
  ...
  const filteredGenders = useMemo(() => {
    if (!search.trim()) return genders;
    const term = search.trim().toLowerCase();
    return genders.filter((gender) =>
      [gender.code, gender.name].some((value) => value?.toLowerCase().includes(term))
    );
  }, [genders, search]);
  ...
  async function loadGenders() {
    ...
    setGenders((data as EmployeeGender[]) || []);
  }

  function openModal(row?: EmployeeGender) {
    setForm({
      id: row?.id || "",
      code: row?.code || "",
      name: row?.name || "",
  ...
  async function handleSave(e: FormEvent) {
    e.preventDefault();
    if (!canManage) {
      setFormError("Only HR/admin can manage employee genders.");
      return;
    }
    ...
    if (error) {
      if (error.code === "23505") {
        setFormError("Gender code or name must be unique for your company.");
      } else {
        setFormError(error.message || "Unable to save employee gender.");
      }
      setSaving(false);
      return;
    }
    ...
    setToast({
      type: "success",
      message: `Employee gender ${form.id ? "updated" : "created"} successfully.`,
    });
    setSaving(false);
    closeModal();
    await loadGenders();
  ...
  if (loading) {
    return <div style={containerStyle}>Loading employee genders…</div>;
  }

  if (!ctx?.companyId) {
    return (
      <div style={containerStyle}>
        <h1 style={{ marginTop: 0 }}>Employee Genders</h1>
    ...
    <div style={containerStyle}>
      <header style={headerStyle}>
        <div>
          ...
          <h1 style={titleStyle}>Employee Genders</h1>
          <p style={subtitleStyle}>Maintain gender options for employee profiles.</p>
        </div>
      ...
      <div style={panelStyle}>
        <div style={panelHeaderStyle}>
          <div>
            <h3 style={{ margin: 0 }}>Genders</h3>
            <p style={{ margin: "4px 0 0", color: "#6b7280", fontSize: 13 }}>
              {filteredGenders.length} genders
            </p>
          </div>
          <input
            value={search}
        ...
        {!filteredGenders.length ? (
          <div style={emptyStateStyle}>
            <h4 style={{ marginTop: 0 }}>No genders found</h4>
            <p style={{ margin: 0, color: "#6b7280" }}>
              {search.trim()
                ? "Try adjusting your search keywords or clear the filter."
                : "Add your first gender to get started."}
            </p>
          </div>
        ) : (
            ...
            <table style={{ width: "100%", borderCollapse: "collapse" }}>
              ...
              <tbody>
                {filteredGenders.map((gender) => (
                  <tr key={gender.id}>
                    <td style={tdStyle}>{gender.code || "—"}</td>
                    <td style={tdStyle}>{gender.name || "—"}</td>
                    <td style={tdStyle}>{gender.sort_order ?? 0}</td>
                    <td style={tdStyle}>{gender.is_active ? "Active" : "Inactive"}</td>
                    <td style={tdStyle}>{formatDate(gender.updated_at)}</td>
                    {canManage ? (
                      <td style={{ ...tdStyle, textAlign: "right" }}>
                        <div style={{ display: "flex", gap: 8, justifyContent: "flex-end" }}>
                          <button onClick={() => openModal(gender)} style={smallButtonStyle}>Edit</button>
                          <button onClick={() => handleToggleStatus(gender)} style={smallButtonStyle}>
        ...
        <div style={modalOverlayStyle}>
          <div style={modalCardStyle}>
            <div style={modalHeaderStyle}>
              <div>
                <h3 style={{ margin: 0 }}>{form.id ? "Edit" : "Add"} Gender</h3>
                <p style={{ margin: "4px 0 0", color: "#6b7280" }}>
                  Keep master data consistent for employee profiles.
                </p>
              </div>
            ...
            <form onSubmit={handleSave} style={formGridStyle}>
              ...
              <div style={{ display: "flex", justifyContent: "flex-end", gap: 12 }}>
                ...
                <button type="submit" style={primaryButtonStyle} disabled={saving}>
                  {saving ? "Saving..." : form.id ? "Save Changes" : "Create Gender"}
                </button>
              </div>
            </form>
          </div>
