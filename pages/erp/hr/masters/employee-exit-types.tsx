...
export default function HrEmployeeExitTypesPage() {
  ...
  const [exitTypes, setExitTypes] = useState<EmployeeExitType[]>([]);
  ...
  const filteredExitTypes = useMemo(() => {
    if (!search.trim()) return exitTypes;
    const term = search.trim().toLowerCase();
    return exitTypes.filter((exitType) =>
      [exitType.code, exitType.name].some((value) => value?.toLowerCase().includes(term))
    );
  }, [exitTypes, search]);
  ...
  async function loadExitTypes() {
    ...
    setExitTypes((data as EmployeeExitType[]) || []);
  }

  function openModal(row?: EmployeeExitType) {
    setForm({
      id: row?.id || "",
      code: row?.code || "",
      name: row?.name || "",
  ...
  async function handleSave(e: FormEvent) {
    e.preventDefault();
    if (!canManage) {
      setFormError("Only HR/admin can manage employee exit types.");
      return;
    }
...
const insertPayload = {
  ...
  const { error } = await supabase.rpc("erp_hr_employee_exit_type_upsert", {
    ...
    if (error) {
      if (error.code === "23505") {
        setFormError("Exit type code or name must be unique for your company.");
      } else {
        setFormError(error.message || "Unable to save employee exit type.");
      }
      setSaving(false);
      return;
    }
    ...
    setToast({
      type: "success",
      message: `Employee exit type ${form.id ? "updated" : "created"} successfully.`,
    });
    setSaving(false);
    closeModal();
    await loadExitTypes();
  ...
  if (loading) {
    return <div style={containerStyle}>Loading employee exit types…</div>;
  }

  if (!ctx?.companyId) {
    return (
      <div style={containerStyle}>
        <h1 style={{ marginTop: 0 }}>Employee Exit Types</h1>
    ...
    <div style={containerStyle}>
      <header style={headerStyle}>
        <div>
          ...
          <h1 style={titleStyle}>Employee Exit Types</h1>
          <p style={subtitleStyle}>Maintain exit type options for employee exits.</p>
        </div>
      ...
      <div style={panelStyle}>
        <div style={panelHeaderStyle}>
          <div>
            <h3 style={{ margin: 0 }}>Exit Types</h3>
            <p style={{ margin: "4px 0 0", color: "#6b7280", fontSize: 13 }}>
              {filteredExitTypes.length} exit types
            </p>
          </div>
          <input
            value={search}
        ...
        {!filteredExitTypes.length ? (
          <div style={emptyStateStyle}>
            <h4 style={{ marginTop: 0 }}>No exit types found</h4>
            <p style={{ margin: 0, color: "#6b7280" }}>
              {search.trim()
                ? "Try adjusting your search keywords or clear the filter."
                : "Add your first exit type to get started."}
            </p>
          </div>
        ) : (
          <div style={{ overflowX: "auto" }}>
            <table style={{ width: "100%", borderCollapse: "collapse" }}>
              ...
              <tbody>
                {filteredExitTypes.map((exitType) => (
                  <tr key={exitType.id}>
                    <td style={tdStyle}>{exitType.code || "—"}</td>
                    <td style={tdStyle}>{exitType.name || "—"}</td>
                    <td style={tdStyle}>{exitType.is_active ? "Active" : "Inactive"}</td>
                    <td style={tdStyle}>{exitType.sort_order ?? 0}</td>
                    {canManage ? (
                      <td style={{ ...tdStyle, textAlign: "right" }}>
                        <div style={{ display: "flex", gap: 8, justifyContent: "flex-end" }}>
                          <button onClick={() => openModal(exitType)} style={smallButtonStyle}>
                            Edit
                          </button>
                          <button
                            onClick={() => handleToggleStatus(exitType)}
                            style={smallButtonStyle}
                          >
                            {exitType.is_active ? "Deactivate" : "Activate"}
                          </button>
                        </div>
                      </td>
                    ) : null}
        ...
        <div style={modalOverlayStyle}>
          <div style={modalCardStyle}>
            <div style={modalHeaderStyle}>
              <div>
                <h3 style={{ margin: 0 }}>{form.id ? "Edit" : "Add"} Exit Type</h3>
                <p style={{ margin: "4px 0 0", color: "#6b7280" }}>
                  Keep exit type data consistent for employee exits.
                </p>
              </div>
            ...
            <form onSubmit={handleSave} style={formGridStyle}>
              ...
              <div style={{ display: "flex", justifyContent: "flex-end", gap: 12 }}>
                ...
                <button type="submit" style={primaryButtonStyle} disabled={saving}>
                  {saving ? "Saving..." : form.id ? "Save Changes" : "Create Exit Type"}
                </button>
              </div>
            </form>
          </div>
