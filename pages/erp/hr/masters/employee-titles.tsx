...
export default function HrEmployeeTitlesPage() {
  ...
  const filteredTitles = useMemo(() => {
    ...
    return titles.filter((title) =>
      [title.code, title.name].some((value) => value?.toLowerCase().includes(term))
    );
  }, [titles, search]);
  ...
  async function loadTitles() {
    ...
    setTitles((data as EmployeeTitle[]) || []);
  }

  function openModal(row?: EmployeeTitle) {
    setForm({
      id: row?.id || "",
      code: row?.code || "",
      name: row?.name || "",
  ...
  async function handleSave(e: FormEvent) {
    e.preventDefault();
    if (!canManage) {
      setFormError("Only HR/admin can manage employee titles.");
      return;
    }
    ...
    if (error) {
      if (error.code === "23505") {
        setFormError("Title code or name must be unique for your company.");
      } else {
        setFormError(error.message || "Unable to save employee title.");
      }
      setSaving(false);
      return;
    }
    ...
    setToast({
      type: "success",
      message: `Employee title ${form.id ? "updated" : "created"} successfully.`,
    });
    setSaving(false);
    closeModal();
    await loadTitles();
  ...
  if (loading) {
    return <div style={containerStyle}>Loading employee titles…</div>;
  }

  if (!ctx?.companyId) {
    return (
      <div style={containerStyle}>
        <h1 style={{ marginTop: 0 }}>Employee Titles</h1>
    ...
    <div style={containerStyle}>
      <header style={headerStyle}>
        <div>
          ...
          <h1 style={titleStyle}>Employee Titles</h1>
          <p style={subtitleStyle}>Maintain title options for employee profiles.</p>
        </div>
            ...
            <table style={{ width: "100%", borderCollapse: "collapse" }}>
              ...
              <tbody>
                {filteredTitles.map((title) => (
                  <tr key={title.id}>
                    <td style={tdStyle}>{title.code || "—"}</td>
                    <td style={tdStyle}>{title.name || "—"}</td>
                    <td style={tdStyle}>{title.sort_order ?? 0}</td>
                    <td style={tdStyle}>{title.is_active ? "Active" : "Inactive"}</td>
                    <td style={tdStyle}>{formatDate(title.updated_at)}</td>
                    {canManage ? (
                      <td style={{ ...tdStyle, textAlign: "right" }}>
                        <div style={{ display: "flex", gap: 8, justifyContent: "flex-end" }}>
                          <button onClick={() => openModal(title)} style={smallButtonStyle}>Edit</button>
                          <button onClick={() => handleToggleStatus(title)} style={smallButtonStyle}>
        ...
        <div style={modalOverlayStyle}>
          <div style={modalCardStyle}>
            <div style={modalHeaderStyle}>
              <div>
                <h3 style={{ margin: 0 }}>{form.id ? "Edit" : "Add"} Title</h3>
