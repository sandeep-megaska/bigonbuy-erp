...
export default function PayrollRunDetailPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const [runResponse, itemsResponse, employeesResponse, statusResponse] = await Promise.all([
        fetch("/api/payroll/runs/get", {
          method: "POST",
          headers: getAuthHeaders(),
          body: JSON.stringify({ runId }),
        }),
        fetch("/api/payroll/items/list", {
          method: "POST",
          headers: getAuthHeaders(),
  ...
  async function loadFinancePostingStatus(currentRunId = runId) {
    ...
    setFinancePostError("");
    try {
      const response = await fetch(`/api/payroll/runs/${currentRunId}/finance-post`, {
        method: "GET",
        headers: getAuthHeaders(),
      });
  ...
  async function openOtDrawer(item) {
    ...
    setOtLoading(true);
    try {
      const response = await fetch("/api/payroll/item-lines/list", {
        method: "POST",
        headers: getAuthHeaders(),
  ...
  async function postFinance() {
    ...
    setFinancePostError("");
    try {
      const response = await fetch(`/api/payroll/runs/${runId}/finance-post`, {
        method: "POST",
        headers: getAuthHeaders(),
        body: JSON.stringify({
  ...
  async function saveOt() {
    ...
    try {
      ...
      const response = await fetch("/api/payroll/item-lines/upsert", {
        method: "POST",
        headers: getAuthHeaders(),
        body: JSON.stringify({
      ...
      const recalcResponse = await fetch("/api/payroll/item/recalculate", {
        method: "POST",
        headers: getAuthHeaders(),
  ...
  async function refreshRun() {
    if (!ctx?.companyId || !runId) return;
    const response = await fetch("/api/payroll/runs/get", {
      method: "POST",
      headers: getAuthHeaders(),
      body: JSON.stringify({ runId }),
    });
