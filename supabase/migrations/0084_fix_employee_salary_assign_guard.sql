-- 0084_fix_employee_salary_assign_guard.sql
-- Prevent backdated effective_from that would make previous assignment invalid.
...
begin
  ...
  if p_salary_structure_id is null then raise exception 'salary_structure_id is required'; end if;
  if p_effective_from is null then raise exception 'effective_from is required'; end if;
  if p_ctc_monthly is null or p_ctc_monthly <= 0 then raise exception 'ctc_monthly must be > 0'; end if;
  ...
  -- Guard against backdating (would violate effective_to >= effective_from after closing)
  if v_prev_id is not null and p_effective_from <= v_prev_from then
    raise exception
      'Invalid effective_from %. It must be after current assignment effective_from % for this employee.',
