create or replace view public.erp_inventory_effective_unit_cost_v as
  ...
  inventory_costs as (
    select
      ...
      bt.base_qty_in,
      case
        when bt.base_qty_in > 0 then (bt.base_cost_total / nullif(bt.base_qty_in, 0))::numeric
        else null
      end as base_unit_cost,
      coalesce(lt.landed_cost_total, 0)::numeric as landed_cost_total,
      case
        when coalesce(bt.base_qty_in, 0) > 0 then coalesce(lt.landed_cost_total, 0) / nullif(bt.base_qty_in, 0)
        when coalesce(oh.on_hand_qty, 0) <> 0 then coalesce(lt.landed_cost_total, 0) / nullif(oh.on_hand_qty, 0)
        else null
      end as landed_unit_cost_delta
    from keys k
  ...
  select
    ...
    ic.base_qty_in,
    ic.base_unit_cost,
    ic.landed_cost_total,
    ic.landed_unit_cost_delta,
    o.unit_cost as override_unit_cost,
    v.cost_price as fallback_cost_price,
    (ic.base_unit_cost + ic.landed_unit_cost_delta) as effective_unit_cost,
    coalesce(o.unit_cost, (ic.base_unit_cost + ic.landed_unit_cost_delta), v.cost_price) as effective_unit_cost_final,
    (
      ic.on_hand_qty
      * coalesce(o.unit_cost, (ic.base_unit_cost + ic.landed_unit_cost_delta), v.cost_price)
    )::numeric as effective_stock_value_final
