-- Fix payroll item recalc to use salary_* fields and only update existing columns
...
begin
  ...
  -- If no row returned, v_basic will still be 0, so explicitly check existence
  if not exists (
    select 1
    from public.erp_payroll_items
    where company_id = v_company_id
