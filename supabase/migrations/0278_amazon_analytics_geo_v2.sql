...
begin
  ...
  ranked as (
    select
      ...
      dense_rank() over (partition by aggregated.state order by aggregated.gross desc) as rank_within_state,
      dense_rank() over (order by aggregated.gross desc) as rank_overall
    from aggregated
  )
  select
    ranked.geo_key,
    ...
    case
      when v_level = 'city' then
        case when ranked.state_gross > 0 then round((ranked.gross / ranked.state_gross) * 100, 2) else 0 end
      else null
    end as gross_share_within_state,
    case when v_level = 'city' then ranked.rank_within_state else null end as rank_within_state,
    ranked.rank_overall
  from ranked
