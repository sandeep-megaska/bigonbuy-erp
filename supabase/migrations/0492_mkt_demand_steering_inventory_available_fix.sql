...
-- Hotfix: erp_inventory_available(null) does NOT return company_id,
-- so we must not reference a.company_id. Company scoping is enforced
-- via join to erp_variants (v.company_id = v_company_id).
...
begin
  ...
  -- FIXED: do not reference a.company_id (not present in function output)
  sku_inventory as (
    select
      v.sku,
  ...
  -- City side unchanged from your existing enhanced function
  with city_sales as (
    select
      src.city,
      src.order_ref,
