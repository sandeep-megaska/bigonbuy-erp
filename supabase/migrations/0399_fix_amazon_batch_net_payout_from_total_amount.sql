-- 0395_fix_amazon_batch_net_payout_from_total_amount.sql
-- Fix: batch net payout should come from payload.summary.total_amount (Amazon report summary row)
...
begin
  ...
  -- FIX: Amazon summary uses total_amount; keep net_payout as fallback for backward compatibility
  begin
    v_batch_net_payout := round(
      coalesce(
        nullif(trim(v_summary ->> 'total_amount'), '')::numeric,
        nullif(trim(v_summary ->> 'net_payout'), '')::numeric,
        0
      ),
      2
    );
  ...
  return jsonb_build_object(
    'batch_id', v_batch_id,
    'attempted_rows', v_attempted_rows,
