-- 0378_fix_expenses_list_posting_state_ambiguous.sql
-- Fix ambiguous reference to posting_state inside RETURNS TABLE function.
-- Forward-only patch.
...
begin
  ...
  select
    ...
    m.is_capitalizable,
    m.applies_to_type,
    m.applied_to_inventory_at,
    m.applied_inventory_ref,
    m.posting_state,
    m.journal_id,
    m.journal_no
  from merged m
  ...
  order by m.expense_date desc, m.created_at desc nulls last;
