-- 0089_fix_leave_lop_and_items_status_v2.sql
-- Fix leave request joins to use leave_type_id and refresh payroll run item status RPC (v2)
-- Also fixes LOP detection: treat leave type code 'LOP' as unpaid (since erp_leave_types has no is_paid)

begin;

-- Patch payroll generate: LOP uses leave_type_id join + LOP code
...
-- Fix leave submit RPC signature (leave_type_id)
...
-- Ensure schema-cache-stable signature exists
...
begin
  ...
  left join public.erp_salary_structures ss
    on ss.company_id = v_company_id
   and ss.id = a.salary_structure_id
