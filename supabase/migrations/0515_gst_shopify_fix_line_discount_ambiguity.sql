-- 0515_gst_shopify_fix_line_discount_ambiguity.sql
-- Fix: erp_gst_generate_shopify had duplicate column name "line_discount" in CTE pricing
-- caused by discount_alloc.* (already contains line_discount) plus computed "as line_discount".
-- We rename computed discount to line_discount_final and update downstream references.
-- No schema/RLS/audit changes.

begin;

create or replace function public.erp_gst_generate_shopify(p_from date, p_to date)
returns jsonb
language plpgsql
security definer
...
begin
  ...
  calc as (
    select
      base.*,
      round(greatest(0, coalesce(base.quantity, 0) * coalesce(base.price, 0)), 2) as line_gross,
      sum(round(greatest(0, coalesce(base.quantity, 0) * coalesce(base.price, 0)), 2)) over (partition by base.order_id)
        as order_line_gross,
  ...
  discount_alloc as (
    select
      ...
      round(calc.order_total_discounts * calc.line_gross / nullif(calc.order_line_gross, 0), 2) as allocated_discount,
      sum(round(calc.order_total_discounts * calc.line_gross / nullif(calc.order_line_gross, 0), 2))
  ...
  pricing as (
    select
      ...
      round(
        least(
          discount_alloc.line_gross,
          case
            when discount_alloc.order_line_discount_total > 0 then coalesce(discount_alloc.line_discount, 0)
            when discount_alloc.order_total_discounts > 0 and discount_alloc.order_line_gross > 0 then
              case
        ...
        2
      ) as line_discount_final
    from discount_alloc
  ),
  calc_total as (
    select
      pricing.*,
      greatest(0, round(pricing.line_gross - pricing.line_discount_final, 2)) as line_taxable,
      sum(greatest(0, round(pricing.line_gross - pricing.line_discount_final, 2)))
  ...
  alloc as (
    select
      ...
      round(
        case
          when calc_total.order_line_total > 0 then (calc_total.order_shipping * calc_total.line_taxable / calc_total.order_line_total)
          else 0
        end,
        2
      ) as shipping_share
  ...
  taxcalc as (
    select
      alloc.*,
      round(alloc.line_taxable * alloc.gst_rate / 100, 2) as line_tax,
      round(alloc.shipping_share, 2) as shipping_taxable,
      round(alloc.shipping_share * alloc.gst_rate / 100, 2) as shipping_tax,
  ...
  select
    ...
    jsonb_build_object(
      'line_gross', taxcalc.line_gross,
      'line_discount', taxcalc.line_discount_final,
