...
begin
  if not exists (
    select 1
    from pg_constraint
    where conname = 'erp_company_settings_gst_state_code_check'
      and conrelid = 'public.erp_company_settings'::regclass
  ) then
    alter table public.erp_company_settings
      add constraint erp_company_settings_gst_state_code_check
      check (gst_state_code is null or gst_state_code ~ '^[0-9]{2}$');
  end if;
end;
$$;

create or replace function public.erp_company_gst_profile()
returns jsonb
language plpgsql
security definer
...
begin
  ...
  select cs.gst_state_code, cs.gst_state_name, cs.gstin
    into v_gst_state_code, v_gst_state_name, v_gstin
    from public.erp_company_settings cs
    where cs.company_id = v_company_id;

  return jsonb_build_object(
    'gst_state_code', v_gst_state_code,
    'gst_state_name', v_gst_state_name,
    'gstin', v_gstin
  );
end;
$$;

revoke all on function public.erp_company_gst_profile() from public;
grant execute on function public.erp_company_gst_profile() to authenticated;
...
begin
  ...
  select cs.gst_state_code, cs.gst_state_name
    into v_gst_state_code, v_gst_state_name
    from public.erp_company_settings cs
    where cs.company_id = v_company_id;
...
create or replace function public.erp_company_update_gst(
  p_gst_state_code text,
  p_gst_state_name text,
...
declare
  ...
  v_gst_state_code text := nullif(trim(coalesce(p_gst_state_code, '')), '');
  v_gst_state_name text := nullif(trim(coalesce(p_gst_state_name, '')), '');
...
begin
  ...
  update public.erp_company_settings
     set gst_state_code = v_gst_state_code,
         gst_state_name = v_gst_state_name,
         gstin = v_gstin,
         updated_by = v_actor
...
revoke all on function public.erp_company_update_gst(text, text, text) from public;
grant execute on function public.erp_company_update_gst(text, text, text) to authenticated;
...
begin
  ...
  select nullif(trim(coalesce(cs.gst_state_code, '')), '')
    into v_company_state_code
    from public.erp_company_settings cs
    where cs.company_id = v_company_id;

  if v_company_state_code is null then
    raise exception 'Company GST state code is not configured. Set it in Company Settings.';
  end if;
...
begin
  ...
  return jsonb_build_object('ok', true, 'doc_no', v_doc_no);
end;
$$;
