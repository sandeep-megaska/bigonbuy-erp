create or replace view public.erp_payroll_eligible_employees_v as
select
  e.company_id,
  e.id as employee_id,
  e.employee_code,
...
where e.company_id = public.erp_current_company_id()
  and e.lifecycle_status in ('active','on_notice');
...
begin
  ...
  for v_employee in
    select employee_id, lifecycle_status
    from public.erp_payroll_eligible_employees_v
    where company_id = v_company_id
  loop
    v_employee_status := v_employee.lifecycle_status;
    if v_employee_status not in ('active', 'on_notice') then
      continue;
    end if;
    ...
    select a.salary_structure_id,
     ...
     where a.company_id = v_company_id
       and a.employee_id = v_employee.employee_id
