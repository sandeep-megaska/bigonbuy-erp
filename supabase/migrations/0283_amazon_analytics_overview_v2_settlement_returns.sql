-- 0283_amazon_analytics_overview_v2_settlement_returns.sql
-- Populate returns from settlement data (Refunds/Returns buckets) and ensure cancellations_value uses row_gross.
...
begin
  ...
  return_totals as (
    select
      coalesce(count(distinct t.order_id) filter (where t.order_id is not null), 0)::bigint as returns_count,
      coalesce(
        sum(abs(coalesce(t.refund_amount, t.net_payout, t.gross_sales, 0)))::numeric,
        0
      ) as returns_value
    ...
    where t.company_id = v_company_id
      ...
      and t.txn_date <= v_to
      and (
        lower(coalesce(t.settlement_type, '')) in ('refunds / returns', 'refunds/returns', 'refunds', 'returns')
        or lower(coalesce(t.settlement_type, '')) like '%refund%'
        or lower(coalesce(t.settlement_type, '')) like '%return%'
      )
  ),
  range_days as (
