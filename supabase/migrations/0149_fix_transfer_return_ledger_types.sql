-- Align transfer/return ledger postings with inventory ledger qty type constraints
...
begin
  ...
  if exists (
    select 1
     ...
     where transfer_id = p_transfer_id
       and company_id = v_company_id
       and (qty <= 0 or qty <> floor(qty))
  ) then
    raise exception 'Transfer line qty must be a positive integer';
  end if;
  ...
  v_ref := coalesce(nullif(trim(v_transfer.reference), ''), p_transfer_id::text);
  ...
  select
    ...
    -abs(l.qty)::integer,
    'adjust_out',
    coalesce(nullif(trim(v_transfer.reference), ''), 'Stock transfer out'),
    v_ref,
    auth.uid(),
    now()
  from public.erp_stock_transfer_lines l
  ...
  select
    ...
    abs(l.qty)::integer,
    'adjust_in',
    coalesce(nullif(trim(v_transfer.reference), ''), 'Stock transfer in'),
    v_ref,
    auth.uid(),
    now()
  from public.erp_stock_transfer_lines l
...
begin
  ...
  if exists (
    ...
    raise exception 'Return receipt line qty must be a positive integer';
  end if;

  v_reason := case
    when v_receipt.receipt_type = 'rto' then 'RTO receipt'
    else 'Return receipt'
  end;
  v_ref := coalesce(nullif(trim(v_receipt.reference), ''), 'RETURN:' || p_receipt_id::text);
