...
select
  ...
  coalesce(a.present_days, 0::numeric(6,2)) as present_days,
  coalesce(a.leave_paid_days, 0::numeric(6,2)) as leave_paid_days,
  coalesce(a.leave_unpaid_days, 0::numeric(6,2)) as leave_unpaid_days,
  coalesce(a.holiday_days, 0::numeric(6,2)) as holiday_days,
  coalesce(a.weekly_off_days, 0::numeric(6,2)) as weekly_off_days,
  coalesce(a.absent_days, 0::numeric(6,2)) as absent_days,
  coalesce(a.unmarked_days, 0::numeric(6,2)) as unmarked_days,
  coalesce(a.payable_days, 0::numeric(6,2)) as payable_days_suggested,
  coalesce(a.lop_days, 0::numeric(6,2)) as lop_days_suggested,
  coalesce(a.period_status, 'missing') as attendance_period_status,
  coalesce(i.payable_days_override, i.payable_days_suggested, a.payable_days, 0::numeric(6,2))
    as payable_days_effective,
  coalesce(i.lop_days_override, i.lop_days_suggested, a.lop_days, 0::numeric(6,2))
