-- 0434_mfg_material_coverage_and_vendor_readiness.sql
-- MFG-PLAN-1: vendor material coverage + ERP vendor readiness indicator

create or replace function public.erp_mfg_material_coverage_v1(
  p_company_id uuid,
  p_vendor_id uuid
) returns table (
  material_id uuid,
  ...
  uom text,
  on_hand_qty numeric,
  demand_qty_next numeric,
  projected_balance numeric,
  shortage_flag boolean,
  days_cover_estimate integer,
...
as $$
  with open_po_lines as (
    ...
    select
      bl.material_id,
      sum(ol.open_qty * bl.qty_per_unit * (1 + coalesce(bl.waste_pct, 0) / 100.0))::numeric as demand_qty_next
    from open_po_lines ol
    join public.erp_mfg_boms b
      on b.company_id = ol.company_id
     and b.vendor_id = ol.vendor_id
     and lower(b.sku) = lower(ol.sku)
     and b.status = 'active'
    ...
    select
      ...
      coalesce(dm.demand_qty_next, 0)::numeric as demand_qty_next,
      (coalesce(mb.on_hand_qty, 0) - coalesce(dm.demand_qty_next, 0))::numeric as projected_balance,
      ((coalesce(mb.on_hand_qty, 0) - coalesce(dm.demand_qty_next, 0)) < 0) as shortage_flag,
      null::integer as days_cover_estimate,
      mb.reorder_point,
      mb.lead_time_days
    from public.erp_mfg_material_balances_v mb
    left join demand_by_material dm
      on dm.material_id = mb.material_id
    where mb.company_id = p_company_id
      and mb.vendor_id = p_vendor_id
      and mb.is_active = true
  ), summary as (
    select
      count(*)::integer as total_materials,
      count(*) filter (where c.shortage_flag)::integer as shortage_count
  ...
  order by c.shortage_flag desc, lower(c.material_name);
$$;
...
begin
  ...
  order by
    case
      ...
      when m.bom_missing_skus > 0 or m.shortage_materials > 0 or m.near_reorder_materials > 0 then 2
      else 3
    end,
    lower(m.vendor_name);
end;
...
revoke all on function public.erp_mfg_material_coverage_v1(uuid, uuid) from public;
revoke all on function public.erp_vendor_readiness_list_v1(uuid, date, date) from public;

grant execute on function public.erp_mfg_material_coverage_v1(uuid, uuid) to service_role;
