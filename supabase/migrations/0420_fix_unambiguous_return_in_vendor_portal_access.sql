-- Fix: remove ambiguous references to output variables (vendor_id/vendor_code)
-- Keep same function signature to avoid return-type change errors.
...
declare
  ...
  l_auth_user_id uuid; -- IMPORTANT: local var, do NOT use output var names
begin
  ...
  do update set
    ...
    updated_at = now(),
    updated_by = auth.uid()
  returning id into l_auth_user_id;  -- ✅ local var, not vendor_id

  update public.erp_vendors v
     set portal_auth_user_id = l_auth_user_id,
         updated_at = now(),
         updated_by = auth.uid()
   where v.id = p_vendor_id
     and v.company_id = p_company_id;
  ...
  select
    ...
    l_vendor_code as vendor_code,
    v_temp_password as temp_password,
    '/mfg/login'::text as login_url;
end;
$$;
