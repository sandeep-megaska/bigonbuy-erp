-- 0280_amazon_analytics_overview_v2.sql
-- Amazon analytics overview v2 (orders/returns tiles)
...
begin
  ...
  with scoped as (
    select
      f.amazon_order_id,
      (coalesce(f.item_amount, 0) + coalesce(f.shipping_amount, 0) + coalesce(f.gift_wrap_amount, 0)) as gross,
  ...
  confirmed_totals as (
    select
      coalesce(count(distinct amazon_order_id), 0)::bigint as orders_count,
      coalesce(sum(gross), 0)::numeric as orders_value,
      coalesce(sum(discount), 0)::numeric as discount_value
    from confirmed
  ),
  cancelled_totals as (
    select
  ...
  range_days as (
    select greatest((v_to - v_from + 1), 0)::int as days_count
  )
  select
    confirmed_totals.orders_value::numeric as gross_sales,
    (confirmed_totals.orders_value - return_totals.returns_value - confirmed_totals.discount_value)::numeric
      as net_sales_estimated,
    confirmed_totals.orders_count::bigint as confirmed_orders_count,
    confirmed_totals.orders_value::numeric as confirmed_orders_value,
    cancelled_totals.orders_count::bigint as cancellations_count,
    cancelled_totals.orders_value::numeric as cancellations_value,
    return_totals.returns_count::bigint as returns_count,
    return_totals.returns_value::numeric as returns_value,
    confirmed_totals.discount_value::numeric as discount_value,
    case
      when (select days_count from range_days) = 0 then 0::numeric
      else (
        confirmed_totals.orders_value - return_totals.returns_value - confirmed_totals.discount_value
      ) / nullif((select days_count from range_days), 0)
    end as avg_per_day,
    (select days_count from range_days)::int as days_count
  from confirmed_totals, cancelled_totals, return_totals, range_days;
end;
$$;
